		...
		for(int i=0; i<8; i++){
			...
			if( vx < m && vx >= 0 && vy < n && vy >= 0 ){
				if ( grid[vx][vy] != -1 ){
					...
					if( y_moved == 0 && x_moved == 0 ){
						path_cost = pow((square(ux-vx) + square(uy-vy)), 0.5);
					}
					if ( ( x_moved == 1 || x_moved == 2) && y_moved == 0 ){
						path_cost = pow((1 + square(uy-vy)), 0.5);
					} 
					if ( ( y_moved == 1 || y_moved == 2) && x_moved == 0){
						path_cost = pow((square(ux-vx) + 1), 0.5);
					}
					if( ( y_moved == 1 || y_moved == 2) && ( x_moved == 1 || x_moved == 2) ){
						path_cost = ROOT;
					}
