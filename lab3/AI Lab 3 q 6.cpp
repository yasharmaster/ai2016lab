...
typedef pair<double, gNODE> iNODE;
...
//	cout << "dijkstra called\n";
	...
	if( grid[sx][sy] > 0 ){
		...
		goal = make_pair(make_pair(sx, sy), gems_vector );
		gem_value = grid[sx][sy];
	}
    
    // Set distance of source to zero and push it to the priority queue
    dist[make_pair(make_pair(sx, sy), gems_vector )] = initial_dist;
    pq.push(make_pair(initial_dist, make_pair(make_pair(sx, sy), gems_vector )));
 //   cout << "source pushd to the queue\n";
    
    // Loop till priority queue is not empty
    while( !pq.empty() ){
    	
    	gNODE popped = pq.top().second;
    	pq.pop();
    	
    	NODE u = popped.first;
		gems_vector = popped.second;
			...
			if( vx < m && vx >= 0 && vy < n && vy >= 0 ){
				if ( grid[vx][vy] != -1 ){
					...
					if( is_collected || grid[vx][vy] == 0 ){
						...
						gNODE child = make_pair(v, gems_vector);
						...
						gNODE child = make_pair(v, temp_gems_vector);
