...
void bfs(Node *source, Node *dest1, Node *dest2, int k, int k_row, int k_col){
	bool found = false;
	int n = source->n;
	queue<Node *> myqueue;
	myqueue.push(source);
		...
		if ( !is_visited(current) ){
			...
			for(int i=0; i<n; i++){
				for(int j=1; j<=n-1; j++){
					...
					if( !is_visited(temp) ){
						temp->parent = current;
						if ( current->k_moved || temp->board[k_row][k_col] != k ){
							temp->k_moved = true;
						}
						myqueue.push(temp);
					}
...
*/
		bfs(source, dest1, dest2, k, k_row, k_col);
	}
	return 0;
}
