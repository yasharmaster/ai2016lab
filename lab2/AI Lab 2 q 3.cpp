...
void shift_rows_rev(Node *node, int row, int times){
	int curr, temp, n = node->n;
	
	while(times--){
		curr = node->board[row][n-1];
		for(int i=n-1; i>=1; i--){
			temp = node->board[row][(i-1)%n];
			node->board[row][(i-1)%n] = curr;
			curr = temp;
		}
		node->board[row][n-1] = temp;
	}
}

void shift_cols_rev(Node *node, int col, int times){
	int curr, temp, n = node->n;
	
	while(times--){
		curr = node->board[n-1][col];
		for(int i=n-1; i>=1; i--){
			temp = node->board[(i-1)%n][col];
			node->board[(i-1)%n][col] = curr;
			curr = temp;
		}
		node->board[n-1][col] = temp;
	}
}
...
void bfs(Node *source, Node *dest, int k){
	bool found = false;
	int n = source->n;
	queue<Node *> myqueue;
	myqueue.push(source);
	...
	while( !myqueue.empty() ){
		...
		if ( !is_visited(current) ){
			...
			// shifting rows reverse
			for(int i=0; i<n; i++){
				
				bool k_present = false;
					
				for (int x=0; x<n; x++){
					if ( current->board[i][x] == k){
						k_present = true;
						break;
					}
				}
